...
pub async fn list_an_organization_quote_s_paginated_teams(configuration: &configuration::Configuration, organization_slug: &str, start_index: Option<i32>, filter: Option<&str>, count: Option<i32>, excluded_attributes: Option<&str>) -> Result<crate::models::ListAnOrganizationSPaginatedTeams200Response, Error<ListAnOrganizationQuoteSPaginatedTeamsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/0/organizations/{organization_slug}/scim/v2/Groups", local_var_configuration.base_path, organization_slug=crate::apis::urlencode(organization_slug));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ListAnOrganizationQuoteSPaginatedTeamsError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn provision_a_new_team(configuration: &configuration::Configuration, organization_slug: &str, provision_a_new_team_request: crate::models::ProvisionANewTeamRequest) -> Result<crate::models::ListAnOrganizationSPaginatedTeams200ResponseResourcesInner, Error<ProvisionANewTeamError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ProvisionANewTeamError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
